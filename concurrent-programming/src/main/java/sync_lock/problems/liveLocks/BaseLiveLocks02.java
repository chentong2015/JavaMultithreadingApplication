package sync_lock.problems.liveLocks;

import java.util.concurrent.locks.Lock;
import java.util.concurrent.locks.ReentrantLock;

// 解决方案2: 将一直hold的锁给释放掉，并且确保一定能释放掉，避免对需要锁的线程造成影响
public class BaseLiveLocks02 {

    /**
     * account1和account2之间的转账会不停的进行，由于无法转账成功，会退回到原来的账户
     * 然后在尝试转账，再次退回到原来的账户，如此持续Live lock状态:
     * Transfer1 and Transfer2 never release the locks they are holding, keep looping
     */
